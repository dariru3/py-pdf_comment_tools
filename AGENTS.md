...
`tests/test_main.py` contains the automated pytest suite. Keep tests centered on temporary, test-generated inputs and outputs rather than committed PDF fixtures. The existing `tests/fixtures/test_keywords.csv` is a small legacy text fixture; do not add new committed test PDFs or generated outputs.
